...
export const validationCreateOffer = [
  ...
  body('sliceDesc').isString().isLength({ min: 10, max: 50 }).trim(),
  ...
  check('currency').isString().isLength({
    min: 1,
    max: 1,
  }).trim().matches(/^[$€₴]$/g),
  body('phoneNumber').isString().isLength({
    min: 10,
    max: 25,
  }).trim().matches(/^[/+]?3?[\s]?8?[\s]?\(?0\d{2}?\)?[\s]?\d{3}[\s|-]?\d{2}[\s|-]?\d{2}$/),
...
export const validationSignUp = [
  ...
  body('userName').isLength({
    min: 2,
    max: 20,
  }).isString().matches(/[а-щА-ЩЬьЮюЯяЇїІіЄєҐґ]/g),
...
export const chatCreateValidate = [
  body('lastId').isNumeric(),
  body('message').isLength({ min: 0, max: 1000 }).isString(),
];

export const createMessageValidate = [
