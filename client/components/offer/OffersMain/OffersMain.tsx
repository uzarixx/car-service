...
import { offersProps, UserRole } from '@/constants/type';
...
const OffersMain: FC<offersProps> = ({
  offers,
  onAddFavorite,
  onDeleteFavorite,
  favorite,
    ...
    <div className={styles.offersWrapper}>
      {offers?.map((el, i) =>
        <div key={i} className={styles.offerBlock}
          ...
          <h4>{el.title}</h4>
          {user && user.role === UserRole.Picker &&
            <div onClick={(e) => e.stopPropagation()}
                 className={styles.favorite}>
              {favorite?.includes(el.id) ?
                <div
                  onClick={() => onDeleteFavorite && onDeleteFavorite(el.id)}>
                  <FavoriteIco />
                </div> :
